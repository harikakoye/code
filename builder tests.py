...
class BuilderListingTest(APITestCase):
    ...
    def setUp(self):
        ...
        listing = Listing.objects.create(title='3bhk on sale',user_id=1,location=location,address=address,propertytype=property_type,transactiontype=transactiontype,property_ownership='co-operative society',description='near park',society_name='101 E san fernando',built_year=1993,posted_by='agent -admin',listing_status='active',occupancy_status='under construction',
        ...
        newbuilderlisting = BuilderListing(builder=newbuilder,venture =newventure,property_type=property_type,price_min=5300000,price_max=8100000,avilable_units=44,description='near to park',features='parking')
        newbuilderlisting.listing.add(listing)
        newbuilderlisting.save()
    def test_builderlisting(self):
        url = baseurl+'/api/builderlisting/?avilable_units=44'
        data = {}
        response = self.client.get(url, data, format='json')
        print json.loads(response.content)
